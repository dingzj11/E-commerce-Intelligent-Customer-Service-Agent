...
| 功能 | 说明 |
|------|------|
| **Flow 流程对话** | 确定性多轮对话管理，LLM 增强意图匹配 |
| **LLM 意图识别** |  无需固定意图分类体系，自然语言理解 |
| **上下文重述** | LLM 重写机器人回复，上下文感知 |
| **多轮槽位填充** |  结构化数据采集，条件分支 |
...
| **Text-to-Cypher** | LLM → 验证 → 校正 → 执行 | 生成和校验 Cypher 查询 |
...
| 功能模块 | 技术实现 | 为什么重要 |
|---------|---------|-----------|
| 分布式锁 | SET NX PX + Lua 原子释放 + 看门狗续期 | 防止多实例并发竞争 |
| 缓存策略 | 三防：穿透(空值缓存)、击穿(互斥锁)、雪崩(TTL 随机化) | 解决经典缓存问题 |
